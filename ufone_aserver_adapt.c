...
void
ufone_double_to_short (short *s, double *f)
{
	int i;
	double max = 65535 / 2;
...
void
ufone_short_to_double (double *f_x, double *f_d, short *s_x, short *s_d)
{
	int i;
	double max = 65535 / 2;
...
void
ufone_adapt_win (double *f_d, double *f_x, double *f_e)
{
	int i;
	for (i = 0; i < LENGTH; i++)
		f_e [i] = nlms_iter (f_x [i], f_d [i], n);
...
{
	short data [LENGTH];
	short adapt_x [LENGTH];
	double f_x [LENGTH], f_d [LENGTH];
	double f_e [LENGTH];
	...
	while ((ret = ufone_alsa_read (data, LENGTH, &ainfo)) == 0)
	{
		...
		ufone_fifo_get (list, &adapt_x[0]);
		ufone_short_to_double (&f_x [0], &f_d [0], &adapt_x [0],
				&data [0]);
		ufone_adapt_win (&f_d [0], &f_x [0], &f_e [0]);
		ufone_double_to_short (&data[0], &f_e[0]);
...
ufone_server_tcp_init (void)
{
	...
	/* Parte da filtragem adaptativa */
	list = fifo_init (LENGTH);

	/*n = nlms_init (50, 0.05, 0.5);*/
	n = nlms_init (100, 0.9, 0.8);
	r = rls_init (50);
